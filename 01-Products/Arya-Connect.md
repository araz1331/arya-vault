...
## Статус
🟢 Готов к продакшену — собран 22–25.09.2026. Единственное, что не работает:
приём платежей, и это упирается в договор с банком, а не в код. См.
[[Kapital-Payments]].
...
Долгоживущее — Hub, голос, почта — на [[Hetzner-Server]].

**Каналы (все шесть работают):** WhatsApp (Embedded Signup + звонки через
[[Arya-Hub]]), веб-виджет, голос (LiveKit, 24/7), Telegram (токен от BotFather),
SMS (LSIM, только исходящие, детект юникода), email (через [[Arya-Mail]],
входящие и исходящие).

**Готово:** OTP-авторизация, база знаний на pgvector (30 документов, 3 языка),
обучение AI на исправлениях менеджера, рассылки (WhatsApp/SMS/email + отложенные
через Cron Trigger), контакты с импортом CSV, аналитика (6 блоков, SQL-функции),
управление шаблонами Meta с медиа и кнопками, автосоздание 6 шаблонов после
Embedded Signup, вебхуки для CRM с подписью HMAC и журналом доставок,
уведомления менеджеру в WhatsApp, приглашение менеджеров, Super Admin
(`/superadmin`), онбординг-визард (`/onboarding`, 3 шага).

## Тарифы — Arya Token (финальная модель)
1 токен = 1 минута голоса. Токены не сгорают.

| Пакет | Цена |
|---|---|
| 100 токенов | 49 AZN |
| 300 токенов | 129 AZN (+10%) |
| 800 токенов | 299 AZN (+23%) |
| 2 000 токенов | 649 AZN (+35%) |

Подписка — 49 AZN/мес, покрывает **все** каналы (раньше продавались по
отдельности по 29/39/99). Пробный период — 7 дней и 50 токенов в подарок,
карта не нужна.

Расход: голос 1 токен/мин, WhatsApp / виджет / Telegram 1 токен за диалог
(окно 24 часа), SMS 0.5 за сегмент, email 0.2 за письмо.

Меньше 10 токенов — уведомление владельцу в WhatsApp и на почту. Ноль —
сервис останавливается: AI молчит, диалог помечается, виджет отдаёт 402.

**`BYPASS_PAYMENT` удалён** (25.09). Это была одна переменная окружения,
пускавшая мимо оплаты *всех* арендаторов сразу. Доступ теперь — факт об
аккаунте: пробный период, подписка или `is_internal`. Миграция 0027 выдала
50 токенов всем существующим арендаторам — без неё переключение заблокировало
бы платформу целиком в момент деплоя.
...
2. ~~**Медиасервер для голоса.**~~ ✅ 22.09 — LiveKit на [[Hetzner-Server]]
   (отдельный coturn не понадобился).
...
5. ~~**Биллинг.**~~ ✅ 25.09 — депозит в манатах заменён на Arya Token:
   подписка 49 AZN/мес на все каналы плюс пакеты токенов, см. тарифы выше.
   Открыт только **приём платежей** — упирается в договор, [[Kapital-Payments]].
...
- [ ] Поиск по диалогам
- [ ] Экспорт данных
- [ ] GDPR: удаление по запросу
- [ ] Реферальная программа
- [ ] Документация API
- [ ] Автопродление подписки (Kapital не поддерживает — сейчас продлевается вручную)
- [ ] SMS: со временем заменить LSIM своим сервисом
- [ ] Лендинг доделан, но есть что улучшить
